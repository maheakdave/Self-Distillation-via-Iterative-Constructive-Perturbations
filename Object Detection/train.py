...
from utils import cos_weights,decay,get_ifgsm,combine_loss
import numpy as np
import random
import torch.backends.cudnn as cudnn
from pycocotools.coco import COCO
from pycocotools.cocoeval import COCOeval
from eval import convert_to_coco_eval_format
...
# Autocast commented out, to avoid NaN issues.
class Trainer:
    def __init__(self,config,model,task_criterion,dist_criterion,train_loader,val_loader,ifgsm_type,epochs,warmup_epochs,iters,target_layers,weighted_feats=True) -> None:
        ...
        self.coco_gt = COCO(self.config.VAL_PATH[1])
    ...
    def _run_epoch(self,epoch):
        ...
        for idx, (xb, yb) in enumerate(self.train_loader):
            ...
            yb = [{key:item[key].cuda() for key in item.keys()} for item in yb]
            ...
            loss_task = self.task_criterion(out)
            
            task_loss_epoch[idx] = loss_task.item()

            if epoch >= self.warmup_epochs:
                self.model.backbone.body.eval()
                ...
                for iteration in range(self.iters):
                    ...
                    out_new = self.model(perturbed_dset,yb)
                    loss = self.task_criterion(out_new)
                    
                # self.model.train()
                self.model.backbone.body.train()
                out_new = self.model(perturbed_dset,yb)
                fmaps_new = self.fmap
                self.fmap = [None] * self.num_layers
                loss_fmap = 0.
    ...
    def validate(self,epoch):
        ...
        for _,(xb,yb) in enumerate(self.val_loader):            
            xb = xb.cuda(non_blocking = True)
            # yb = yb.cuda(non_blocking = True)
            image_ids = [t["image_id"] for t in yb]
            yb = [
                {
                    k: (v.cuda() if k != 'image_id' else v)
                    for k, v in item.items()
                }
                for item in yb
            ]
            out_og = self.model(xb)
            results = convert_to_coco_eval_format(out_og, image_ids)
            coco_predictions.extend(results)
            self.fmap = [None] * len(self.target_layers)
            torch.cuda.empty_cache()
            _ = gc.collect()
        with open("coco_results.json", "w") as f:
            json.dump(coco_predictions, f)
        coco_dt = self.coco_gt.loadRes("coco_results.json")
...
def main(config,ifgsm_type,epochs,warmup_rate,iters,weighted_feats):
    ...
    target_layers = [getattr(model.backbone.body,name) for name in ['layer1','layer2','layer3','layer4']]
