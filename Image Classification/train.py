...
from utils import cos_weights,decay,get_ifgsm,AverageMeter,timer,modify_resnet50
import numpy as np
import random
...
class Trainer:
    ...
    def validate(self,epoch):
        ...
        for _,(xb,yb) in enumerate(self.val_loader):            
            ...
            self.loss_og += loss
            self.accuracy.update(out_og,yb)
            self.precision.update(out_og,yb)
            self.recall.update(out_og,yb)
            self.f1.update(out_og,yb)
...
def load_train_objs(config):
    ...
    # model = convnext_base(num_classes = config.num_classes)
...
def main(config,ifgsm_type,epochs,warmup_rate,iters,weighted_feats):
    ...
    # target_layers = [getattr(model,name) for name in config.target_layers]
