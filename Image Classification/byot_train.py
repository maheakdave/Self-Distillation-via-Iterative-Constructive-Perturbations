...
import gc
import time
from dataset import build_cifar100_loader
from config import Config
import torch 
from torchmetrics.classification import MulticlassAccuracy,MulticlassRecall,MulticlassPrecision,MulticlassF1Score
from torchvision.models import resnet50,convnext_base
import os, platform
from torch.amp import GradScaler,autocast
from utils import cos_weights,decay,get_ifgsm,AverageMeter,timer,modify_resnet50,ResNetSelfDistillationWrapper,distillation_loss,ConvNextSelfDistillationWrapper
import numpy as np
import random
...
class Trainer:
    def __init__(self,config,model,task_criterion,dist_criterion,train_loader,val_loader,ifgsm_type,epochs,warmup_epochs,iters,target_layers,weighted_feats=True) -> None:
        ...
        self.accuracy = MulticlassAccuracy(num_classes=config.num_classes).to(self.device)
        self.precision = MulticlassPrecision(num_classes=config.num_classes).to(self.device)
        self.recall = MulticlassRecall(num_classes=config.num_classes).to(self.device)
        self.f1 = MulticlassF1Score(num_classes=config.num_classes).to(self.device)
    ...
    def _run_epoch(self,epoch):
        ...
        for idx, (xb, yb) in enumerate(self.train_loader):
            ...
            with autocast(device_type='cuda', enabled=True):
                ...
                loss_task = self.task_criterion(*out,yb)
            ...
            if epoch >= self.warmup_epochs:
                ...
                perturbed_dset = xb
                # with autocast(device_type='cuda', enabled=True):
                loss = self.task_criterion(*self.model(perturbed_dset), yb)
                self.fmap = [None] * self.num_layers
                ...
                for iteration in range(self.iters):
                    ...
                    out_new = self.model(perturbed_dset)
                    loss = self.task_criterion(*out_new, yb)
    ...
    def validate(self,epoch):
        ...
        for _,(xb,yb) in enumerate(self.val_loader):            
            ...
            out_og = self.model(xb)
            self.fmap = [None] * len(self.target_layers)
            loss = self.task_criterion(*out_og,yb)
            self.loss_og += loss
            self.accuracy.update(out_og[1],yb)
            self.precision.update(out_og[1],yb)
            self.recall.update(out_og[1],yb)
            self.f1.update(out_og[1],yb)

            torch.cuda.empty_cache()
            _ = gc.collect()

        self.ac = self.accuracy.compute()
        self.pr = self.precision.compute()
        self.re = self.recall.compute()
        self.f1score = self.f1.compute()

        self.test_time[epoch] = time.time() - test_start
        
        print(f"\n------------METRICS------------")
        print(f"ACCURACY = {(self.ac*100):.2f}")
        print(f"PRECISION = {(self.pr):.4f}")
        print(f"RECALL = {(self.re):.4f}")
        print(f"F1-Score = {(self.f1score):.4f}")
...
def load_train_objs(config):
    data_loader_train, data_loader_val = build_cifar100_loader()
    print(f"Loading training objects\n")
    model = resnet50(num_classes = config.num_classes)
    model = modify_resnet50(model) if config.modify_model else model
    # model = convnext_base()
    task_criterion = distillation_loss
...
def main(config,ifgsm_type,epochs,warmup_rate,iters,weighted_feats):
    ...
    target_layers = [getattr(model,name) for name in ['layer1','layer2','layer3','layer4']]
    # target_layers = [getattr(model.features,str(i)) for i in range(0,8,1)]
    model = ResNetSelfDistillationWrapper(model=model,target_layers=target_layers)
    # model = ConvNextSelfDistillationWrapper(model=model,target_layers = target_layers)
